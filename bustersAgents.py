...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostDist in livingGhostPositionDistributions:
            mostLikelyPos = max(ghostDist.items(), key=lambda pair: pair[1])[0]
            distToGhost = self.distancer.getDistance(pacmanPosition, mostLikelyPos)
            if min_Dist > distToGhost:
                min_Dist = distToGhost
                min_Ghost_Pos = mostLikelyPos

        min_Dist = float('inf')
        bestAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            distToGhost = self.distancer.getDistance(min_Ghost_Pos, successorPosition)
            if min_Dist > distToGhost:
                min_Dist = distToGhost
                bestAction = action
        return bestAction
